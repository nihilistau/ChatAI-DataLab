...
| --- | --- | --- | --- | --- |
| Backend unit/integration tests | FastAPI routes, schemas, services under `relay/backend` | `pytest` (already in `requirements.txt`) | ```pwsh
cd relay/backend
python -m pytest -q
``` | Run locally before pushing and on every PR (CI). |
| Backend contracts | 1) Request/response shapes vs. frontend expectations 2) DB migrations | `pytest -m contract` (add marker) + schema diff via Alembic (future) | Define fixtures in `relay/backend/tests/conftest.py` to load canned payloads and assert response JSON. | Nightly or before releasing. |
| Backend quality gates | Import errors, type hints, formatting | `python -m compileall relay/backend`, `ruff check` once added to toolchain | Add to CI once linting config lands. | CI + pre-commit. |
| Frontend smoke tests | Rendering of critical React components, API client stubs | `vitest` / `npm test -- --runInBand` (once suite added) | Seed with snapshots for `CanvasBoard`, `TailLogCell`, etc. Run commands from `relay/frontend`. | Before merging UI changes. |
...
| Workshop validations | Notebook helpers (`workshop/scripts/*.py`), metrics sanity checks | `pytest workshop/scripts` + `papermill` smoke for key recipes | ```pwsh
cd workshop
...
- Use the datastore fixture in `relay/backend/tests/conftest.py` (in-memory SQLite) to keep unit tests isolated while still exercising `data_store_context`.
...
- Capture anonymized golden payloads (JSON) under `relay/backend/tests/data/` so the same request corpus powers regression tests, Workshop recipes, and API documentation.
...
| Pre-release artifact | `Save-LabWorkspace -Destination "../releases/relay-workshop-v0.5.0.zip"` | Checked-in `releases/` dir or external blob storage. |
...
### 2.3 Workshop artifacts

- Save recipes (notebooks) with embedded outputs into `workshop/notebooks/exports/` and commit only sanitized versions (PII scrubbed).
- Use `papermill` or `nbconvert` to render HTML/PDF snapshots and upload alongside workspace archives.
- When recipes depend on large intermediate files, stash them in `data/artifacts/<date>/` and include that folder in `Save-LabWorkspace -Include relay,workshop,data,scripts` (already default).
