...
| Restart a service | `Restart-LabJob -Name workshop` | Convenience wrapper for stop + start. |
| Start everything | `Start-AllLabJobs` | Spins up backend, frontend, and Workshop concurrently. |
...
> 🧭 Prefer the new noun-first aliases (`Workshop-Job-Start`, `Workshop-Job-Show`, etc.) when documenting or sharing commands—the LabControl module exports them as aliases for the existing `*-Lab*` cmdlets so naming stays consistent with Workshop terminology.
...
| Persist snapshot to disk | `Save-LabFolderSnapshot -Group workshop -Destination data/integrity/workshop-latest.json` | Useful before risky refactors; outputs path to saved JSON for audit logs. |
| Validate against saved snapshot | `Test-LabFolderSnapshot -SnapshotPath data/integrity/workshop-latest.json` | Re-computes hashes and emits drift report; fails the pipeline early if the tree diverged. |

### 1.3 Workspace backups

| Scenario | Command | Output |
| --- | --- | --- |
| Ad-hoc zip backup | `Save-LabWorkspace -Include relay,workshop,data,scripts` | Produces `backups/workspace-YYYYMMDD-HHMMSS.zip` by default. |
...
## Relay data-store CLI

Need a quick peek at recent chat traffic, artifacts, or the tail log without wiring up ad-hoc SQL queries? The new `scripts/relay_store.py` helper speaks to the same multi-provider store that powers the backend (SQLite, JSON snapshot, or Cosmos DB) and emits either pretty text or JSON that you can pipe elsewhere.

| Command | Purpose | Notes |
| --- | --- | --- |
| `python scripts/relay_store.py interactions --limit 15` | Dumps the most recent prompts + model metadata. | Add `--json` to feed downstream data tooling. Honors provider + credentials from `.env`. |
| `python scripts/relay_store.py artifacts --json` | Lists the latest artifacts as structured JSON. | Great for exporting manifest summaries or Ops reports without hitting the HTTP API. |
| `python scripts/relay_store.py create-artifact --title "Weekly pulse" --body "..." --owner ops` | Inserts an artifact directly into the store. | Accepts `--category` / `--accent`; returns the record as JSON when `--json` is set. |
| `python scripts/relay_store.py tail-log --limit 5 --follow` | Streams the newest log lines that power the Ops Deck tail. | `--follow` polls the active provider instead of tailing the raw SQLite file under `data/`, so Cosmos + JSON modes keep working. Pair with `tail-log-add` (below) for one-off notes. |
| `python scripts/relay_store.py tail-log-add "preflight complete" --source buildbot` | Appends a tail log entry. | Useful when pipelines or scripts need to annotate the deck without going through the HTTP API. |
| `python scripts/relay_store.py summary` | Prints the active provider plus quick counts. | Handy smoke test after flipping `DATABASE_PROVIDER`. |

> 💡 Run the CLI from any shell (PowerShell, bash, etc.). On Windows you can pin helper aliases like `function Get-RelayInteractions { python scripts/relay_store.py interactions --limit 25 }` inside your profile for faster access.

**Notebook automation tie-in.** Papermill runs launched via `python scripts/control_center.py notebook --db-path auto` inherit the same datastore provider. Keep `--db-path auto` (or omit the flag) so notebooks can hop between SQLite, JSON snapshots, and Cosmos without editing parameters. Pair each scheduled run with `python scripts/relay_store.py tail-log-add "papermill · control_center_relay completed" --source automation` so the ops tail log captures notebook execution.
...
| --- | --- | --- | --- |
| Manifest validator (onboarding) | `python scripts/manifest_validator.py configs/relays/onboarding.json --json --expect-tenant demo-tenant --expect-relay welcome-control` | `manifest`, `mcp`, `validator` | Emits the JSON summary used by MCP/Chromatic workflows so agents can validate manifests without leaving the Ops Deck. |
...
| Workshop · start all jobs | `pwsh -NoProfile -Command "Import-Module ./scripts/powershell/LabControl.psm1 -DisableNameChecking; Workshop-Job-StartAll"` | `workshop`, `jobs`, `start` | Triggers every registered Lab job via the noun-first Workshop alias (wraps `Start-AllLabJobs`). |
| Workshop · stop all jobs | `pwsh -NoProfile -Command "Import-Module ./scripts/powershell/LabControl.psm1 -DisableNameChecking; Workshop-Job-StopAll"` | `workshop`, `jobs`, `stop` | Gracefully halts all running jobs; catalog version omits `-Force` for safety. |
| Workshop · job dashboard | `pwsh -NoProfile -Command "Import-Module ./scripts/powershell/LabControl.psm1 -DisableNameChecking; Workshop-Job-Show"` | `workshop`, `jobs`, `status` | Lists each job name, PowerShell job id, and timestamps—built for Ops Deck quick looks. |
| Workshop · job snapshot | `pwsh -NoProfile -Command "Import-Module ./scripts/powershell/LabControl.psm1 -DisableNameChecking; Workshop-Job-Snapshot"` | `workshop`, `jobs`, `snapshot` | Captures the manifest snapshot object used by Ops telemetry tooling (wraps `Get-LabJobSnapshot`). |
...
- Pass `-ChangelogTemplate docs/CHANGELOG_TEMPLATE.md -ChangelogSections "Highlights","Ops","Workshop"` to generate structured release notes (tokens `{{VERSION}}` / `{{DATE}}` are replaced during templating).
...
### Ops workflow harness (composite run)

When you need the full “release dry-run → goals log → Master Change scaffold → rebrand dry-run → integrity checkpoint” flow, call the workflow harness directly or via Control Center. Every run now emits a tail-log entry (`scripts/relay_store.py tail-log-add … --source workflow-harness`) and appends a JSON payload to `data/logs/release.log`, giving Ops Deck widgets immediate telemetry. Opt out with `--skip-tail-log` or `--skip-release-log` when rehearsing locally:

- **Direct CLI:** `python scripts/workflow_harness.py --notes "Ops request" --goal-milestone "Ops workflow harness" --goal-summary "Scripted release/doc/rebrand run" --goal-artifacts "`scripts/workflow_harness.py`" --timeline-json data/logs/workflow_harness/latest.json --timeline-jsonl data/logs/workflow_harness/history.jsonl`
- **Control Center wrapper:** `python scripts/control_center.py workflow -- --skip-release --timeline-json data/logs/workflow_harness/latest.json --timeline-jsonl data/logs/workflow_harness/history.jsonl` (add any flags after the `--`).
- **Ops Deck command:** the catalog entry “Ops workflow harness” now includes the timeline flags so MCP jobs persist the result automatically.

The `--timeline-json` (overwrites) and `--timeline-jsonl` (appends) switches capture the structured step-by-step output for replay in Ops Deck widgets, automation logs, or future MCP commands. Pair them with the default tail-log + release-log hooks (or provide `--tail-log-message` / `--tail-log-source` for custom context) and keep everything inside `data/logs/` for easy diffing.

## 8. Search telemetry ingestion pipeline

- `Update-LabSearchTelemetry` calls `python scripts/search_telemetry.py ingest --log-path logs/search-history.jsonl --output data/search_telemetry.json --runs-parquet data/search_telemetry-runs.parquet --daily-parquet data/search_telemetry-daily.parquet`, hashes every JSONL entry, and writes both the JSON ledger plus optional Parquet extracts (install `pyarrow` via `pip install -r workshop/requirements.txt`). Pass `--no-tail-log` when you need to suppress datastore tail-log events during dry runs.
...
- The companion notebook `workshop/notebooks/search_telemetry.ipynb` plots sweep volume vs. findings and exposes flakiness density. Parameterize it with `SEARCH_LEDGER_PATH` (Papermill already does this inside `tests/test_notebooks.py`).
...
Set-Location -Path 'd:\Files\Code 3\ChatAI-DataLab\relay\frontend'
npm run test
``` |
