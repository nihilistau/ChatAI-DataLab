...
1. Launch Jupyter Lab (`cd workshop && . .venv/bin/activate && jupyter lab`).
...
2. Modify `relay/backend/app/models.py` intentionally (add whitespace).
3. `python scripts/project_integrity.py verify relay/backend/app/models.py` shows the hash mismatch.
4. `python scripts/project_integrity.py repair relay/backend/app/models.py --checkpoint latest` restores the pristine copy.
...
- **Workshop assets**: drop additional recipes (notebooks) into `workshop/notebooks/` and document them inside `docs/TUTORIALS.md` so future contributors know how to reproduce your analysis (recipe and asset system is implemented; future tutorials will cover new additions).
