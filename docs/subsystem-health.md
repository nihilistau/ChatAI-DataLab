...
| Workshop / DataLab parity | Papermill CLI ✅ — `control_center_relay-cli-20251117-052129.ipynb` + `control_center_snapshot.json` emitted under `workshop/notebooks/_papermill/` (archived copies remain in `legacy/datalab`). Workshop shims still backed by pytest. | Medium-High | Reference the snapshot JSON when triaging Ops signals; ensure `workshop/requirements.txt` stays in sync with datalab shim and schedule next run after manifest changes.
| Documentation | README + Agent Playbook updated through Workshop migrations; new audit log recorded here. | High | Link this file from `docs/OPERATIONS_HANDBOOK.md` under health checks section.
| Integrity tooling | `python scripts/project_integrity.py status` reports 120 added / 32 deleted / 65 modified files (mostly artifacts + Workshop migration files). | Medium | Curate ignore list or checkpoint once new baseline is intentional; ensure dist/storybook outputs aren’t committed before tagging.

## Control Center telemetry snapshot

- Latest run: `workshop/notebooks/_papermill/control_center_relay-cli-20251117-052129.ipynb`
- Tail log + service summary: `workshop/notebooks/_papermill/control_center_snapshot.json`
