...
```
<repo>/
├── relay/             # Horizon Relay frontend + backend apps
├── controlplane/            # Cross-platform orchestrator
├── workshop/                 # Active notebooks, scripts, metrics helpers
├── legacy/archives/         # Read-only historical notebooks/scripts
...
| --- | --- | --- | --- |
| `relay/frontend` | UI | React components, styles, lib APIs | Only TypeScript/TSX; tag files with `@tag:ui` + feature-specific tags |
| `relay/backend` | API | FastAPI app, SQLAlchemy models, services | Python only; keep modules under 400 LOC and use section banners |
| `controlplane` | Ops | Orchestrators for Ops Deck, CLI, PowerShell | Avoid hard-coded paths; tag `@tag:ops` |
| `workshop` | Insights | Notebooks, scripts, metrics helpers | Keep notebooks in `notebooks/`; helper modules under `scripts/` |
| `legacy/archives` | Historical reference | Archived copies of notebooks/scripts from earlier runs | Read-only; reference only when porting older work |
