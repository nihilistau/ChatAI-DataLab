...
"""ORM models backing the Playground storage layer."""
# @tag:backend,models
...
from sqlalchemy import JSON, Column, DateTime, Integer, String, Text, ForeignKey, UniqueConstraint
...
# --- Playground manifest storage -------------------------------------------
class PlaygroundManifest(Base):
    """Versioned manifest history for each tenant + Playground namespace."""

    __tablename__ = "playground_manifests"
    __table_args__ = (
        UniqueConstraint("tenant", "playground", "revision", name="uq_playground_revision"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    tenant: Mapped[str] = mapped_column(String(64), nullable=False)
    playground: Mapped[str] = mapped_column(String(64), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    revision_label: Mapped[str | None] = mapped_column(String(64), nullable=True)
    manifest = Column(JSON, nullable=False)
    cookbook: Mapped[str | None] = mapped_column(String(160), nullable=True)
    recipe: Mapped[str | None] = mapped_column(String(160), nullable=True)
