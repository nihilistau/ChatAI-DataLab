"""Workshop-native helpers for relay state snapshots and health checks."""
...
def save_relay(manifest_path, snapshot_path):
...
def load_relay(snapshot_path, manifest_path):
...
def auto_snapshot(manifest_path="configs/relays/onboarding.json", snapshot_path="data/relay-onboarding-snapshot.json"):
    try:
        save_relay(manifest_path, snapshot_path)
        print("[AutoSnapshot] Relay state updated.")
    except (OSError, IOError, ValueError) as e:
        print(f"[AutoSnapshot] Relay snapshot failed: {e}")
...
# Multi-relay/user support

def list_manifests(config_dir="configs/relays"):
