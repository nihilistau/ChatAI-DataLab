...
"""Helper utilities for publishing Relay manifests from the Workshop."""
...
DEFAULT_BASE_URL = os.getenv("RELAY_API_URL", "http://localhost:8000/api")
...
class ManifestPublisher:
    ...
    ----------
    base_url:
        Base API endpoint (defaults to the RELAY_API_URL environment variable).
    token:
    ...
    def __init__(
        ...
        self.token = token or os.getenv("RELAY_API_TOKEN")
    ...
    def publish(
        ...
        *,
        ...
        response = self._request(
            "POST",
            f"/relays/{tenant}/{relay}/manifests",
        ...
        return ManifestPublishResult(
            ...
            url=f"{self.base_url}/relays/{tenant}/{relay}/manifests/{data['revision']}",
            manifest=data["manifest"],
        )

    def latest(self, *, tenant: str, relay: str) -> dict[str, Any] | None:
        response = self._request(
            "GET", f"/relays/{tenant}/{relay}/manifests/latest", allow_404=True
        )
