...
"""Tests for the Control Center API surface powering the Relay."""
...
def test_trigger_notebook_job(client: TestClient, notebook_runner_stub) -> None:
    response = client.post(
        ...
        json={"name": "control_center_relay.ipynb", "parameters": {"DB_PATH": "sqlite:///tmp.db"}},
    )
    assert response.status_code == 202
    assert notebook_runner_stub.submit_calls[0][0] == "control_center_relay.ipynb"