...
"""REST routes bridging the Relay stack, Ops Deck, and the Tail log."""
# @tag:backend,api,ops
...
from ..services.release_log import list_release_log_entries
...
@router.get("/release-log", response_model=list[ReleaseLogEntryRead])
def list_release_log(limit: int = Query(12, ge=1, le=200)):
    entries = list_release_log_entries(limit)
    return [ReleaseLogEntryRead.model_validate(entry) for entry in entries]
