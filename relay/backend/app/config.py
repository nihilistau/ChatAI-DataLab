...
DEFAULT_RELEASE_LOG_PATH = PROJECT_ROOT / "data" / "logs" / "release.log"
...
class Settings(BaseSettings):
    ...
    database_provider: Literal["sqlite", "cosmos", "json"] = Field(
        default="sqlite", alias="DATABASE_PROVIDER"
    )
    database_path: Path = Field(default=DEFAULT_DB_PATH, alias="DATABASE_PATH")
    json_store_path: Path = Field(
        default=PROJECT_ROOT / "data" / "relay_store.json",
        alias="JSON_STORE_PATH",
    )
    ...
    cosmos_interaction_container: str = Field(
        default="relay-interactions",
        alias="COSMOS_INTERACTIONS_CONTAINER",
    )
    cosmos_artifact_container: str = Field(
        default="relay-artifacts",
        alias="COSMOS_ARTIFACTS_CONTAINER",
    )
    cosmos_tail_log_container: str = Field(
        default="relay-tail-log",
        alias="COSMOS_TAIL_LOG_CONTAINER",
    )
    ...
    release_log_path: Path = Field(default=DEFAULT_RELEASE_LOG_PATH, alias="RELEASE_LOG_PATH")
