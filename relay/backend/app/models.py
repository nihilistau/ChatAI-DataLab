...
"""ORM models backing the Relay storage layer."""
# @tag:backend,models
...
# --- Relay manifest storage -------------------------------------------
class RelayManifest(Base):
    """Versioned manifest history for each tenant + Relay namespace."""

    __tablename__ = "relay_manifests"
    __table_args__ = (
        UniqueConstraint("tenant", "relay", "revision", name="uq_relay_revision"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    tenant: Mapped[str] = mapped_column(String(64), nullable=False)
    relay: Mapped[str] = mapped_column(String(64), nullable=False)
