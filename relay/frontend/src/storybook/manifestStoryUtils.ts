...
export interface ManifestStoryKnobs {
  ...
  sections: RelayLayoutSection[];
  actions: RelayAction[];
}

const baseCreatedAt = Date.now() - 1000 * 60 * 60;

const defaultSections: RelayLayoutSection[] = [
  {
    id: "intel",
    title: "Intel stack",
  ...
  },
  {
    id: "actions",
    title: "Automation",
    description: "Trigger Ops or Workshop routines wired to manifest actions.",
    widgets: [
      {
        id: "notebook",
        type: "notebook-trigger",
        title: "Run control_center_relay",
        props: {
          label: "Notebook",
          notebook: "control_center_relay.ipynb",
...
const defaultActions: RelayAction[] = [
  {
    id: "deploy-control",
    title: "Deploy Control Center",
    method: "POST",
    route: "/api/control/deploy",
    description: "Runs the orchestrator relay and syncs notebooks."
  },
  {
    id: "refresh-search",
...
export const defaultManifestKnobs: ManifestStoryKnobs = {
  ...
  metadata: {
    hero: "Workshop publishes manifest revisions that hydrate the Control Center shell.",
    notes: "Adjust widget mix to match the telemetry lanes under review.",
    tags: ["ops", "manifest", "workshop"]
  },
  sections: defaultSections,
  actions: defaultActions
};

const cloneSections = (sections: RelayLayoutSection[]): RelayLayoutSection[] =>
...
const cloneActions = (actions: RelayAction[]): RelayAction[] =>
  actions.map((action, index) => ({
    ...action,
    id: action.id ?? `action-${index}`
  }));

export function buildManifestRecord(knobs?: Partial<ManifestStoryKnobs>): RelayManifestRecord {
  const args: ManifestStoryKnobs = { ...defaultManifestKnobs, ...knobs };
  return {
    id: `${args.tenant}-${args.relay}-rev${args.revision}`,
    tenant: args.tenant,
    relay: args.relay,
