...
function ManifestWidgetSummary() {
  const { manifest, loading, error, status, tenant, relay } = useManifest();
    ...
    <section className="intel-card manifest-summary-card">
      ...
      {!loading && manifest && (
        <>
          ...
          {widgets.length === 0 && <p className="text-muted">No widgets defined yet—publish from Workshop to hydrate.</p>}
          {widgets.length > 0 && (
