...
import ReleaseLogPanel from "./components/ReleaseLogPanel";
...
function App() {
  ...
  const [releaseLogEntries, setReleaseLogEntries] = useState<ReleaseLogEntry[]>([]);
  const [releaseLogLoading, setReleaseLogLoading] = useState(false);
  const [releaseLogError, setReleaseLogError] = useState<string | null>(null);
  ...
  const refreshReleaseLog = useCallback(async () => {
    setReleaseLogLoading(true);
    try {
      const entries = await fetchReleaseLog();
      setReleaseLogEntries(entries);
      setReleaseLogError(null);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Failed to load release log";
      setReleaseLogError(message);
    } finally {
      setReleaseLogLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.localStorage.setItem("relay.theme", themeKey);
    }
  }, [themeKey]);
  ...
  useEffect(() => {
    ...
    appendTailLog(`manifest · ${manifest.relay} rev ${manifest.revision} synced`, "manifest");
    ...
    <main className="app-shell dark">
      ...
      <section className="manifest-panel">
        <header className="panel-header">
          <div>
            <p className="eyebrow">Relay manifest</p>
            <h2>Workshop ↔ Control surface</h2>
          </div>
          <div className="panel-controls">
            <small>
              {manifestTenant}/{manifestRelay}
            </small>
        ...
        <p className="lead">Latest layout published from the Workshop. Update the Welcome Cookbook to refresh this preview.</p>
      ...
      <div className="workspace-grid">
        ...
        <div className="intel-stack">
          ...
          <ReleaseLogPanel
            entries={releaseLogEntries}
            loading={releaseLogLoading}
            error={releaseLogError}
            onRefresh={refreshReleaseLog}
          />
        </div>
      </div>
