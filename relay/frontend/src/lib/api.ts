...
const mapReleaseTimelineEntry = (input: any) => ({
  name: input.name ?? "step",
  status: input.status ?? "unknown",
  timestamp: normalizeOptionalTimestamp(input.timestamp ?? input.at ?? input.time),
  startedAt: normalizeOptionalTimestamp(input.started_at ?? input.startedAt),
  endedAt: normalizeOptionalTimestamp(input.ended_at ?? input.endedAt),
  durationSeconds: input.duration_seconds ?? input.durationSeconds ?? undefined,
  details: input.details ?? undefined
});

const mapReleaseLogEntry = (input: any): ReleaseLogEntry => ({
  id: input.id,
  timestamp: normalizeTimestamp(input.timestamp),
  kind: input.kind ?? "release_pipeline",
  ...
  checkpointId: input.checkpoint_id ?? input.checkpointId ?? undefined,
  releaseDir: input.release_dir ?? input.releaseDir ?? undefined,
  releaseMode: input.release_mode ?? input.releaseMode ?? undefined,
  releaseTag: input.release_tag ?? input.releaseTag ?? undefined,
  checkpointTag: input.checkpoint_tag ?? input.checkpointTag ?? undefined,
  goalMilestone: input.goal_milestone ?? input.goalMilestone ?? undefined,
  changeMasterId: input.change_master_id ?? input.changeMasterId ?? undefined,
  ...
  timeline: (Array.isArray(input.timeline) ? input.timeline : []).map(mapReleaseTimelineEntry)
});

const mapRelayManifestRecord = (input: any): RelayManifestRecord => ({
  id: input.id,
  tenant: input.tenant,
  relay: input.relay,
...
export async function fetchReleaseLog(limit = 6): Promise<ReleaseLogEntry[]> {
  const data = await apiFetch<any[]>(`/api/release-log?limit=${limit}`);
  return data.map(mapReleaseLogEntry);
}
...
export async function fetchLatestRelayManifest(
  tenant: string,
  relay: string
): Promise<RelayManifestRecord | null> {
  const data = await apiFetchNullable<any>(`/api/relays/${tenant}/${relay}/manifests/latest`);
  return data ? mapRelayManifestRecord(data) : null;
}
