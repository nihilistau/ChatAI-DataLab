...
const mapRelayManifestRecord = (input) => ({
    id: input.id,
    tenant: input.tenant,
    relay: input.relay,
...
export async function fetchLatestRelayManifest(tenant, relay) {
    const data = await apiFetchNullable(`/api/relays/${tenant}/${relay}/manifests/latest`);
