# v1.0.3-control-capsules.20251118 — Workshop-first telemetry bundle
...
## Highlights
- **Notebook relocation, validated.** All operational notebooks now live under `workshop/notebooks/`. The release bundle ships the freshly executed Workshop versions so downstream teams stop depending on the archived DataLab tree.
...
- python -m papermill workshop/notebooks/search_telemetry.ipynb workshop/notebooks/_papermill/search_telemetry-release.ipynb -p SEARCH_LEDGER_PATH data/search_telemetry.json -p TELEMETRY_LOG_PATH logs/search-history.jsonl
- python -m papermill workshop/notebooks/ops_response_playbook.ipynb workshop/notebooks/_papermill/ops_response_playbook-release.ipynb -k python3
- python -m papermill workshop/notebooks/widget_showcase.ipynb workshop/notebooks/_papermill/widget_showcase-release.ipynb -k python3
...
5. Notebook evidence (Workshop snapshots):
   - `notebooks/search_telemetry-executed.ipynb`
   - `notebooks/ops_response_playbook.ipynb`
   - `notebooks/widget_showcase.ipynb`

## Operator callouts
- All manifest-driven notebooks now reference `workshop.*` helpers. Mention the relocation in release notes so external collaborators stop editing `legacy/datalab/notebooks/*`.
...
## Next steps
- Update the remaining automation + tests (e.g., `tests/test_notebooks.py`) to execute Workshop notebooks directly so CI stops depending on the archived DataLab copies.
