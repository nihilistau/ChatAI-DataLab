...
- **Scopes** reference top-level areas (`backend`, `frontend`, `workshop`, `scripts`, `docs`, `labctl`).
...
Reference CI jobs exactly as defined under `.github/workflows/workshop-notebooks.yml` when discussing failures:
`backend-tests`, `workshop-tests`, `frontend-qa`, `storybook-builds`, `integrity-scan`.
